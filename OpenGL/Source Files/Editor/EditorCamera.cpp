...
namespace Editor {
	...
	Camera::Camera(glm::vec3 position, glm::vec3 up, float yaw, float pitch, float fov, float nearDis, float farDis)
	...
	void Camera::set_aspect(float size) {
		m_aspectRatio = size;
		/*int width, height;
	...
	void Camera::on_mouse_move(float deltaYaw, float deltaPitch, bool constrainPitch) {
	...
	void Camera::UpdateZoom(float offset, Shader& shader) {
