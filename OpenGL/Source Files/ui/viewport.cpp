...
void ui::Viewport::render()
{
	...
	mMesh["cube1"]->setPosition(glm::vec3(0.0f, 10.0f, 0.0f));
	...
	float angle = glfwGetTime() * rotationSpeed;  // Get elapsed time and multiply by speed
	m_model = glm::rotate(m_model, glm::radians(angle), glm::vec3(1.0f, 0.0f, 0.0f));  // Rotate around the X-axis
